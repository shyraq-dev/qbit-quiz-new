...
-- ── BUILTIN QUESTIONS кестесі ─────────────────────────────
-- Астаналар, Тулар, 4 сурет 1 сөз ойындары үшін
CREATE TABLE IF NOT EXISTS builtin_questions (
  id         uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  slug       text        NOT NULL,  -- 'capitals' | 'flags' | '4pics'
  question   text        NOT NULL,
  answer     text        NOT NULL,
  flag       text,                  -- emoji flag (flags үшін)
  images     jsonb,                 -- URL массиві (4pics үшін)
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_builtin_slug ON builtin_questions(slug);

ALTER TABLE builtin_questions ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='builtin_questions' AND policyname='anon read builtin') THEN
    CREATE POLICY "anon read builtin" ON builtin_questions FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='builtin_questions' AND policyname='service write builtin') THEN
    CREATE POLICY "service write builtin" ON builtin_questions FOR ALL USING (auth.role()='service_role') WITH CHECK (auth.role()='service_role');
  END IF;
END $$;

-- Үлгі деректер
INSERT INTO builtin_questions (slug, question, answer, flag) VALUES
  ('capitals', 'Қазақстан', 'Астана', null),
  ('capitals', 'Франция', 'Париж', null),
  ('capitals', 'Жапония', 'Токио', null),
  ('flags', 'Қазақстан', 'Қазақстан', '🇰🇿'),
  ('flags', 'Германия', 'Германия', '🇩🇪'),
  ('flags', 'Жапония', 'Жапония', '🇯🇵')
ON CONFLICT DO NOTHING;

NOTIFY pgrst, 'reload schema';

-- ══════════════════════════════════════════════════════════
-- Builtin games questions table (Астаналар, Тулар, 4 сурет)
-- ══════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS builtin_questions (
  id         uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  slug       text        NOT NULL,   -- 'capitals' | 'flags' | '4pics'
  question   text        NOT NULL,
  answer     text        NOT NULL,
  flag       text,                   -- flags ойыны үшін emoji
  images     jsonb,                  -- 4pics үшін URL массиві
  created_at timestamptz NOT NULL DEFAULT now()
);
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='builtin_questions' AND policyname='service write builtin') THEN
    CREATE POLICY "service write builtin" ON builtin_questions FOR ALL
...
-- ── BUILTIN GAMES QUESTIONS ──────────────────────────────
-- Астаналар, Тулар, 4 сурет 1 сөз үшін
create table if not exists builtin_questions (
  id         uuid primary key default gen_random_uuid(),
  slug       text not null,          -- 'capitals' | 'flags' | '4pics'
  question   text not null,          -- Сұрақ (ел аты немесе т.б.)
  answer     text not null,          -- Жауап (астана немесе т.б.)
  flag       text,                   -- Тулар үшін emoji немесе URL
  image_url  text,                   -- 4pics үшін сурет URL
  created_at timestamptz not null default now()
);
...
-- Үлгі деректер — Астаналар
insert into builtin_questions (slug, question, answer) values
  ('capitals', 'Қазақстан', 'Астана'),
  ('capitals', 'Ресей',     'Мәскеу'),
  ('capitals', 'Франция',   'Париж'),
  ('capitals', 'Германия',  'Берлин'),
  ('capitals', 'Жапония',   'Токио'),
  ('capitals', 'Қытай',     'Пекин'),
  ('capitals', 'АҚШ',       'Вашингтон'),
  ('capitals', 'Ұлыбритания','Лондон')
on conflict do nothing;
...
insert into builtin_questions (slug, question, answer, flag) values
  ...
  ('flags', 'Ресей',       'Ресей',        '🇷🇺'),
  ('flags', 'АҚШ',         'АҚШ',          '🇺🇸'),
  ('flags', 'Франция',     'Франция',      '🇫🇷'),
  ('flags', 'Германия',    'Германия',     '🇩🇪'),
  ('flags', 'Жапония',     'Жапония',      '🇯🇵'),
  ('flags', 'Қытай',       'Қытай',        '🇨🇳'),
  ('flags', 'Ұлыбритания', 'Ұлыбритания',  '🇬🇧')
on conflict do nothing;

-- Schema cache жаңарту
notify pgrst, 'reload schema';

-- ── BUILTIN QUESTIONS — жаңа columns (егер кесте бар болса) ─
alter table builtin_questions add column if not exists wrong_answers jsonb;
alter table builtin_questions add column if not exists images        jsonb;
alter table builtin_questions add column if not exists letter_count  integer;

notify pgrst, 'reload schema';

-- ── GAME PLAYERS — correct/wrong count columns ────────────
alter table game_players add column if not exists correct_count integer not null default 0;
alter table game_players add column if not exists wrong_count   integer not null default 0;
