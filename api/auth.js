...
async function verifySession({ token }) {
  ...
  // #2 FIX: is_admin — ADMIN_ID арқылы нақты тексеру
  const isAdmin = !!(ADMIN_ID && decoded.uid === `tg:${ADMIN_ID}`);
  if (isAdmin !== user.is_admin) {
    await supabase.from('users').update({ is_admin: isAdmin }).eq('id', decoded.uid);
    user.is_admin = isAdmin;
  }

  console.log(`[verifySession] uid=${decoded.uid} isAdmin=${isAdmin}`);
